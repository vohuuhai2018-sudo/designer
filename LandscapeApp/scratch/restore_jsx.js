...
const brokenReg = /\{\s*selectedProject\.finalImage\s*&&\s*\([\s\S]*?<img className="final-preview-image" src=\{selectedProject\.finalImage\} alt="Bản vẽ hoàn thiện" \/>\s*<\/div>\s*\)\}\s*<button\s*key=\{option\.id\}/m;

c = c.replace(brokenReg, missingBlock + `                      <button
                        key={option.id}`);
