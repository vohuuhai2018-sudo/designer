...
.done-eyebrow-line {
  width: 40px;
  height: 1.5px;
  background: linear-gradient(to right, var(--bronze-400), transparent);
  opacity: 0.6;
}
...
// Replace the old block from .done-hero to .done-sub
const oldCssBlock = /\.done-hero\s*\{[\s\S]*?\.done-sub\s*\{[\s\S]*?\}/;
css = css.replace(oldCssBlock, newStyles);

fs.writeFileSync(cssPath, css, 'utf8');
console.log('SUCCESS: Redesigned done-hero section for better desktop/mobile appearance');
